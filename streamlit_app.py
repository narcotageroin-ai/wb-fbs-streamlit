...
with st.sidebar:
    st.header("Настройки")
    token = st.text_input("WB API Token", value=os.getenv("WB_API_TOKEN", ""), type="password", help="Токен категории Marketplace")
    env = st.selectbox("Среда", ["prod", "sandbox"], index=0 if os.getenv("WB_ENV","prod")=="prod" else 1,
...
with tab_pipeline:
    ...
    st.session_state["active_supply"] = st.text_input("Текущая поставка (supplyId)", value=st.session_state["active_supply"])

    auto_print = st.checkbox("Автогенерация стикера (PNG 58x40)", value=True)
    auto_meta_before_add = st.checkbox("Сначала записывать метаданные, затем добавлять заказ в поставку", value=True)

    scan_val = st.text_input("Поле для сканера", value="", help="Сфокусируйте курсор и сканируйте штрихкод/строку")
    ...
    def _parse_scan(s: str):
        ...
        if len(parts) > 1:
            ...
            if len(payload) in (14, 15, 16) and payload.isdigit():
                ...
                meta["uin"] = payload
            elif len(payload) > 22:  # условно для SGTIN (длинная строка)
                meta["sgtin"] = payload
            else:
                meta["gtin"] = payload
    ...
    if scan_val:
        ...
        if not order_id:
            st.error("Не распознан orderId в начале строки. Формат: <orderId>|<метка>")
        else:
            try:
                # метаданные до добавления в поставку (для маркируемой продукции)
                ...
                # добавить заказ в поставку (перевод в confirm)
                ...
                # метаданные после (если выбран другой режим)
                ...
                # автосоздание стикера
                if auto_print:
                    content = wb_api.get_stickers([order_id], fmt="png", width=58, height=40)
                    st.download_button("Скачать стикер (PNG)", data=content, file_name=f"{order_id}_sticker.png")
...
with tab_orders:
    ...
    if st.button("Получить новые задания"):
        try:
    ...
    with col1:
        date_from = st.date_input("Дата с", datetime.utcnow().date()-timedelta(days=7))
    with col2:
        date_to = st.date_input("Дата по", datetime.utcnow().date())
    if st.button("Загрузить заказы"):
        import calendar, time as _t
        ts_from = int(time.mktime(datetime(date_from.year, date_from.month, date_from.day).timetuple()))
        ts_to = int(time.mktime(datetime(date_to.year, date_to.month, date_to.day, 23,59,59).timetuple()))
        try:
...
with tab_supply:
    ...
    office_id = st.number_input("destinationOfficeId (опционально, обязательно с 01.09.2025 для совпадения officeID заказов)", min_value=0, value=0)
    if st.button("Создать поставку"):
        try:
            payload = {}
            if office_id:
                payload["destinationOfficeId"] = office_id
    ...
    supply_id = st.text_input("ID поставки", value=st.session_state.get("supply_id",""))
    order_id_to_add = st.number_input("Добавить заказ (orderId)", min_value=0, value=0, step=1)
    if st.button("Добавить заказ в поставку"):
        try:
    ...
    with c1:
        if st.button("Показать заказы поставки"):
            try:
                st.json(wb_api.get_supply_orders(supply_id))
            except Exception as e:
                st.error(str(e))
    with c2:
        if st.button("Передать поставку в доставку"):
            try:
                ...
                st.error(str(e))
    with c3:
        if st.button("Получить QR поставки (png)"):
            try:
                img_bytes = wb_api.get_supply_qr(supply_id, fmt="png")
                st.download_button("Скачать QR поставки", data=img_bytes, file_name=f"{supply_id}_qr.png")
    ...
    cols = st.columns(4)
    with cols[0]:
        amount = st.number_input("Добавить коробов", min_value=1, value=1)
        if st.button("Добавить короба"):
            try:
                st.json(wb_api.add_boxes(supply_id, amount))
            except Exception as e:
                st.error(str(e))
    with cols[1]:
        if st.button("Список коробов"):
            try:
                st.json(wb_api.get_supply_boxes(supply_id))
            except Exception as e:
                st.error(str(e))
    with cols[2]:
        trbx_ids_str = st.text_input("trbxIds через запятую", "")
        if st.button("Стикеры коробов (png)"):
            try:
                trbx_ids = [x.strip() for x in trbx_ids_str.split(",") if x.strip()]
                content = wb_api.get_box_stickers(supply_id, trbx_ids, fmt="png")
                st.download_button("Скачать стикеры коробов", data=content, file_name=f"{supply_id}_trbx_stickers.png")
                st.image(content, caption="Стикеры коробов")
            except Exception as e:
                st.error(str(e))
    with cols[3]:
        if st.button("Удалить короба (по trbxIds)"):
            try:
                trbx_ids = [x.strip() for x in trbx_ids_str.split(",") if x.strip()]
...
with tab_labels:
    ...
    order_ids_str = st.text_area("orderIds (через запятую)")
    fmt = st.selectbox("Формат", ["png", "svg", "zplv", "zplh"], index=0)
    size = st.selectbox("Размер", ["58x40", "40x30"], index=0)
    if st.button("Сгенерировать стикеры"):
        try:
            ...
            st.download_button("Скачать стикеры", data=content, file_name=f"stickers.{ext}")
...
with tab_meta:
    ...
    order_id = st.number_input("orderId", min_value=0, value=0, step=1)
    colm = st.columns(5)
    with colm[0]:
        sgtin = st.text_input("SGTIN")
        if st.button("Добавить SGTIN"):
            try:
                ...
                st.error(str(e))
    with colm[1]:
        uin = st.text_input("UIN")
        if st.button("Добавить UIN"):
            try:
    ...
    with colm[3]:
        ...
        if st.button("Добавить GTIN"):
            try:
                ...
                st.error(str(e))
    with colm[4]:
        exp = st.text_input("Срок годности (YYYY-MM-DD)")
        if st.button("Добавить срок годности"):
            try:
    ...
    if st.button("Показать метаданные заказа"):
        try:
...
with tab_pass:
    ...
    if st.button("Получить офисы, где требуется пропуск"):
        try:
    ...
    with c2:
        ...
        office_id = st.number_input("officeId", min_value=0, value=0)
        car = st.text_input("Госномер", "")
        date_from = st.date_input("Дата с", datetime.utcnow().date())
        date_to = st.date_input("Дата по", datetime.utcnow().date())
        driver = st.text_input("Водитель (опционально)", "")
        if st.button("Создать пропуск"):
            try:
